...
class ObsExpSnipperTriu(cooltools.api.snipping.ObsExpSnipper):
    """
    O/E snipper for triu snips
    old way to delet other half
    """
    ...
    def select(self, region1, region2):
        ...
        if self.cooler_opts["sparse"]:
            matrix = matrix.tocsr()
        # Filling in the lower triangular with np.nan:
...
def pValue_featureOccurenceInLoop(
    ...
    for i in range(N_shuffle):
        ...
        inter_shuffle = (
            ncRNA_andFeature.intersect(las_loops_shuffled, wa=True, f=fraction)
            .to_dataframe()
            .drop_duplicates()
        )
    ...
    inter_lasLoops = (
        ncRNA_andFeature.intersect(df_loops, wa=True, f=fraction).to_dataframe().drop_duplicates()
    )
    if return_df:
...
def pValue_featureJaccardLoop(
    ...
    # load las-loops
    df_loops = load_BedInMode(file_loops, time, mode, resolution=resolution)
    df_loops.loc[:, ["chrom", "start", "end"]].sort_values(by=["chrom", "start"]).to_csv(
        "data/genome/tmp.bed", sep="\t", index=False, header=False
    )
    df_loops = BedTool("data/genome/tmp.bed")
    ...
    for i in range(N_shuffle):
        las_loops_shuffled = df_loops.shuffle(g=genome_file, chrom=True, seed=i)
        inter_shuffle = BedTool.jaccard(ncRNA_andFeature, BedTool.sort(las_loops_shuffled))['jaccard']
        inter_shuffle_vec.append(inter_shuffle)
    inter_shuffle_df = pd.DataFrame({"Shuffle": inter_shuffle_vec})
    inter_lasLoops = BedTool.jaccard(ncRNA_andFeature, df_loops)['jaccard']
    if return_df:
        return inter_shuffle_df, inter_lasLoops
    else:
        print(inter_lasLoops)
        fig = sns.histplot(
            data=inter_shuffle_df, x="Shuffle", kde=True, #stat="percent", binwidth=0.1
        )
        fig.axvline(inter_lasLoops, color="red", lw=3)
        p_value = np.round(
            np.min(
                [
                    len(
                        inter_shuffle_df[
                            inter_shuffle_df["Shuffle"] > inter_lasLoops
                        ]
                    )
                    / N_shuffle,
                    len(
...
def prepare_view_df(clr: cooler.Cooler) -> pd.DataFrame:
    """
    Prepare a dataframe to be used as the `view_df` argument for `cooltools.expected_cis`
    
    Args:
        clr (cooler.Cooler): A cooler object to get the chromsizes from.
        
    Returns:
        pd.DataFrame: A dataframe with columns 'chrom', 'start', 'end', 'name'
    """
    df_chromsizes = clr.chromsizes.reset_index()
    df_chromsizes.columns = ['chrom', 'end']
    df_chromsizes.loc[:, 'start'] = 0
    ...
    df_chromsizes['end'] = clr.chromsizes.tolist()
    df_chromsizes = df_chromsizes[['chrom', 'start', 'end', 'name']]
    return df_chromsizes

def plot_stack_mtx(stack, flank, resolution, resolution_label='bp', file_name=None, vmax=None, vmin=None):
    """
    Plots the stack of snips and shows the log2 mean observed interactions
    ...
    stack : np.ndarray
        The stack of snips.
    flank : int
        The number of basepairs on either side of the snips.
    resolution : int
        The bin resolution of the snips.
    resolution_label : str, optional
        The label to use for the x-axis ticks. Default is 'bp'.
    file_name : str, optional
        The name of the file to save the plot to. If None, the plot is displayed
        without saving. Default is None.
    vmax : float, optional
        The maximum value to use for the colorbar. If None, the maximum value
        is determined from the data. Default is None.
    vmin : float, optional
        The minimum value to use for the colorbar. If None, the minimum value
        is determined from the data. Default is None.
    """

    # Calculate the mean observed interactions for each bin
    mtx = np.nanmean(stack, axis=2)

    # Plot the log2 mean observed interactions
    plt.imshow(
        np.log2(mtx),
        vmax=vmax,
        vmin=vmin,
    ...
    # Add a colorbar with the label 'log2 mean obs/exp'
    ...
    plt.xticks(ticks_pixels, ticks_kbp)
    plt.yticks(ticks_pixels, ticks_kbp)
    plt.xlabel(f'relative position, {resolution_label}')
    plt.ylabel(f'relative position, {resolution_label}')
...
def create_pileup_and_save_plot(clr, conv_pairs_TPM, df_chromsizes, expected_df, group_length, file_name):
    """
    Create a pileup for features defined by 'both high', 'sum gene high', 'intergenic high', 'both low'
    and anchor_group == 'loop anchors'
    and save the plot as a PDF in the results/pics/paper directory
    """
    # Create the pileup
    pup = coolpup.pileup(clr, conv_pairs_TPM, features_format='bed', view_df=df_chromsizes, expected_df=expected_df, flip_negative_strand=True, mindist=1_500, maxdist = 50_000, by_distance=[1_500, 10_000, 20_000, 30_000, 40_000, 50_000], flank=10_000, min_diag=0, nproc=4) #, groupby=['name1', 'name2']
    # Plot the pileup
    plotpup.plot(pup, score=True, cmap='coolwarm', scale='log', sym=True, vmax=1.5, vmin=0.8, height=3)
    # Save the plot as a PDF
    plt.savefig("results/pics/paper/"+file_name+".pdf", dpi=100,bbox_inches='tight')
    # Close the plot
    plt.show()
    plt.clf()
...
def lineplot_plusplus(orientation = "horizontal", **kwargs):
    ...
    :param orientation: The orientation of the line plot. Default is "horizontal".
    :param **kwargs: Additional keyword arguments to be passed to the sns.lineplot() function.

    :return: The line plot with modified orientation and axis labels.
    """
